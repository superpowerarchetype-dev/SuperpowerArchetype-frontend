...
export default function Page() {
  ...
  return (
    <div className={cn(notoThai.className, "relative flex flex-col justify-between bg-[#5D5D5D] items-center w-full h-screen overflow-hidden text-white px-[20px] ")}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0 }} className="absolute top-0 left-0 flex h-[48px] w-full items-center px-[20px] z-15">
        <Link href="/Q6" className="flex flex-row items-center text-white">
          <ChevronLeft color="white" />
          <h1 className={cn(anuphan.className, "text-[15px] ml-1")}>{t("back")}</h1>
        </Link>
      </motion.div>

      <motion.div initial={{filter: "blur(0px)"}} animate={{ filter: "blur(3px)"}} transition={{ duration: duration, delay: 1 }} className="absolute bottom-[10%] h-full w-full z-5">
        <Image src="/img/Q7.webp" alt="scene" fill className="h-[70%] object-contain z-5" priority />
      </motion.div>
      
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: duration, delay: 1 }} className="flex flex-col w-full justify-center pt-[88px] text-center text-white flex-1 z-10">
        <h1 className="text-[15px] ">{t("peopleTalk")}</h1>
        <h1 className="text-[17px] font-semibold pt-[20px]">{t("everyoneShare")}</h1>
        <h1 className="text-[17px] font-semibold ">{t("resistSilence")}</h1>
        <h1 className="text-[17px] font-semibold ">{t("whichStory")}</h1>
        ...
        <div className="flex flex-col items-center justify-center text-center py-[20px] text-[18px] font-light z-10 w-full gap-y-[20px]">
            ...
            <div className={cn(anuphan.className,`flex flex-col items-center w-full h-full border-2 border-[#0A0A0A] text-center transition-all duration-150 rounded-[4px] ${isAHold ? 'bg-[#D1D1D1] border-b-2' : 'bg-white border-b-4'}`)}>
                <button className="flex flex-col items-center justify-center w-full h-full text-black rounded-[4px] text-center py-[12px] px-[8px] text-[15px] font-medium"
                    ...
                    <h1>{t.rich("choiceA_1", { span: (chunks) => <span className="font-bold">{chunks}</span> })}</h1>
                    <h1>{t("choiceA_2")}</h1>
                    <h1>{t("choiceA_3")}</h1>
                    <h1>{t("choiceA_4")}</h1>
                </button>
            ...
            <div className={cn(anuphan.className,`flex flex-col items-center w-full h-full border-2 border-[#0A0A0A] text-center transition-all duration-150 rounded-[4px] ${isBHold ? 'bg-[#D1D1D1] border-b-2' : 'bg-white border-b-4'}`)}>
                <button className="flex flex-col items-center justify-center w-full h-full text-black rounded-[4px] text-center py-[12px] px-[8px] text-[15px] font-medium"
                    ...
                    <h1>{t.rich("choiceB_1", { span: (chunks) => <span className="font-bold">{chunks}</span> })}</h1>
                    <h1>{t("choiceB_2")}</h1>
                    <h1>{t("choiceB_3")}</h1>
                    <h1>{t("choiceB_4")}</h1>
                </button>
            ...
            <div className={cn(anuphan.className,`flex flex-col items-center w-full h-full border-2 border-[#0A0A0A] text-center transition-all duration-150 rounded-[4px] ${isCHold ? 'bg-[#D1D1D1] border-b-2' : 'bg-white border-b-4'}`)}>
                <button className="flex flex-col items-center justify-center w-full h-full text-black rounded-[4px] text-center py-[12px] px-[8px] text-[15px] font-medium"
                    ...
                    <h1>{t.rich("choiceC_1", { span: (chunks) => <span className="font-bold">{chunks}</span> })}</h1>
                    <h1>{t("choiceC_2")}</h1>
                    <h1>{t("choiceC_3")}</h1>
                    <h1>{t("choiceC_4")}</h1>
                </button>
