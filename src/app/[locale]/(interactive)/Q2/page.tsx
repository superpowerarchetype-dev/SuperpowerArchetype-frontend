...
export default function Page() {
  ...
  return (
    <div className={cn(notoThai.className, "relative flex flex-col justify-between items-center w-full h-screen overflow-hidden bg-[#F0F0F0] text-white px-[20px]")}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: duration, delay: 1 }} className="pt-[68px] font-semibold text-[17px] text-center text-[#0A0A0A]">
        <h1>{t("whyYou")}</h1>
        <h1>{t("whyNow")}</h1>
        <h1>{t("heartBeat")}</h1>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0 }} className="flex flex-col justify-center items-center flex-1 w-full">
        <div className="relative w-full max-w-md h-full">
          <Image src="/img/Q2.webp" alt="scene" fill className="object-contain" priority />
        </div>
      </motion.div>
