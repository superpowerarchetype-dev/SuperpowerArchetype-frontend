...
export default function Page() {
  ...
  return (
    <div className={cn(notoThai.className, "relative flex flex-col justify-between items-center w-full h-screen overflow-hidden bg-[#2C2C2C] text-white px-[20px]")}>
      ...
      <motion.div className="flex flex-col justify-center items-center flex-1 w-full pt-[48px]">
        <div className="relative w-full max-w-md h-full ">
          <Image src="/img/scene1-4.webp" alt="scene" fill className="object-cover" priority />
        </div>
      </motion.div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0 }} className="flex flex-col items-center justify-center text-center py-[20px] text-[15px] font-light z-10 w-full">
        <h1>{t("whisperPromise")}</h1>
        <h1 className="font-semibold text-[18px] py-[1%] ">{t("peaceOrder")}</h1>
        <h1>{t("yearning")}</h1>
        <h1>{t("peopleAccepted")}</h1>
        {locale === 'en' && 
          <h1>they accepted it.</h1>
        }
      </motion.div>
