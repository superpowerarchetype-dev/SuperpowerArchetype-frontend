...
export default function Page() {
    ...
    return (
    <div className={cn(notoThai.className, "flex flex-col min-h-screen w-full bg-[#F0F0F0] text-[#0A0A0A] px-[20px]")}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.1, delay: 0 }} className="flex flex-col min-h-screen flex-glow">
            ...
            <div className="flex flex-col flex-1 pt-[68px]">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 2 }} className="h-[408px] flex-1 relative w-full overflow-hidden border-3 border-black">
                    <motion.div initial={{ x: "-60%" }} animate={{ x: "-30%" }} transition={{delay:2.5, duration: 4, ease: "easeInOut" }} className="absolute top-0 left-0 h-full flex">
                        <div className="relative w-[2000px] h-full mx-[20px]">
                            <Image src="/img/scene2-1.webp" alt="Scrolling Scene" fill className="object-contain" />
                        </div>
                    </motion.div>
                </motion.div>
                ...
                { state === 1 && 
                    <div className="flex flex-col justify-center py-[20px] text-[15px] font-light z-10 w-full">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 1 }} className="pb-[20px]">
                            <h1>{t("artistContact")}</h1>
                        </motion.div>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 2 }}>
                            <h1>{t("wordsThought")}</h1>
                            <h1>{t("artNotDie")}</h1>
                        </motion.div>
                    </div>
                }
                { state === 2 && 
                    <div className="flex flex-col justify-center py-[20px] text-[15px] font-light z-10 w-full">  
                        ...
                        {
                          locale === 'en' &&
                          <h1>whether red, yellow, or blue.</h1>
                        }
                    </div>
                }
            </div>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 2 }} className="flex flex-col py-[20px] h-[88px] w-full items-center z-10">
                {state === 1 && 
