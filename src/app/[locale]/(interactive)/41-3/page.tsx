    ...
    <div className={cn(notoThai.className, "relative flex flex-col justify-between items-center w-full h-screen overflow-hidden bg-[#5D5D5D] text-white px-[20px]")}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0 }} className="flex flex-col flex-1 w-full">
        <div className="absolute top-0 left-0 flex h-[48px] w-full items-center px-[20px]">
            <Link href="/Q7" className="flex flex-row items-center text-white">
            <ChevronLeft color="white" />
            <h1 className={cn(anuphan.className, "text-white text-[15px] ml-1")}>{t("back")}</h1>
            </Link>
        </div>
        <motion.div className="flex flex-col items-left justify-center text-left pt-[88px] w-full">
            <h1 className="text-[18px] font-semibold">{t("iHeard")}</h1>
            <h1 className="text-[18px] font-semibold">{t("nextPlan")}</h1>
            {
                locale === 'en' &&
                <h1 className="text-[18px] font-semibold">device, “Its next move!”</h1>
            }
        </motion.div>
        <div className="flex flex-col flex-1 w-full">
            <div className="flex flex-col flex-1 justify-center">
                <motion.div className="flex flex-col flex-1 justify-center items-center h-[300px] w-full ">
                    ...
                    <Image src="/img/scene4-3.webp" alt="scene" fill className="object-cover" priority />
                    </div>
                </motion.div>

                <motion.div className="flex flex-col  items-left justify-center text-left py-[20px] text-[18px] z-10 w-full">
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: duration, delay: 1 }}>
                        <h1 className="">{t("silenceRelease")}</h1>
                        <h1>{t("brainwash")}</h1>
                    </motion.div>
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: duration, delay: 2 }} className="pt-[40px] gap-y-[4px]">
                        <h1 className="text-[15px]">{t("onlyWay")}</h1>
                        <h1 className="font-semibold text-[18px]">{t("destroyTower")}</h1>
                        <h1 className="text-[15px]">{t("centerCity")}</h1>
                    </motion.div>
                </motion.div>
            </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: duration, delay: 2 }} className="flex flex-col py-[20px] h-[88px] w-full items-center z-10">
            <Link href="/Q8" className="h-[48px] w-full">
