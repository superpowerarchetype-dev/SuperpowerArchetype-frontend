...
export default function Page() {
  ...
  const handleAnswer = (input:string) => {
    localStorage.setItem("Q10",input);
    router.push("/5-1");
  }
  ...
  return (
    <div className={cn(notoThai.className, "relative flex flex-col justify-between bg-[#F4F4F4] items-center w-full h-screen overflow-hidden text-[#0A0A0A] px-[20px] ")}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0 }} className="absolute top-0 left-0 flex h-[48px] w-full items-center px-[20px] z-15">
        <Link href="/Q9" className="flex flex-row items-center text-[#0A0A0A]">
          <ChevronLeft color="#0A0A0A" />
          <h1 className={cn(anuphan.className, "text-[15px] ml-1")}>{t("back")}</h1>
        </Link>
      </motion.div>

      <motion.div initial={{filter: "blur(0px)"}} animate={{ filter: "blur(3px)"}} transition={{ duration: duration, delay: 1 }} className="absolute bottom-[0%] h-full w-full z-5">
        <Image src="/img/Q10.webp" alt="scene" fill className="h-[70%] object-cover z-5" priority />
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: duration, delay: 1 }} className="flex flex-col w-full justify-center pt-[88px] text-center flex-1 z-10">
        <h1 className="text-[17px] font-semibold">{t("teamDiscouraged")}</h1>
        <h1 className="text-[17px] font-semibold ">{t("whatToSay")}</h1>
