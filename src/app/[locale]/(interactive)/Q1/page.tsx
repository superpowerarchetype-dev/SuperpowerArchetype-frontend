...
import { Link } from "~/lib/navigation"; // ใช้ Link จาก lib/navigation
...
export default function Page() {
  ...
  return (
    <div className={cn(
      ...
      >
        <Link href="/1-8" className="flex flex-row items-center text-white">
          <ChevronLeft color="white" />
          <h1 className={cn(anuphan.className, "text-white text-[15px] ml-1")}>
            {t("back")}
          </h1>
        </Link>
      </motion.div>
      ...
      <motion.div
        ...
        className="pt-[68px] font-semibold text-[17px] text-center"
      >
        <h1>{t("questionLine1")}</h1>
        <h1>{t("questionLine2")}</h1>
      </motion.div>
      ...
      >
        {/* Choice A */}
        <div className={cn(anuphan.className,`flex flex-col items-center w-full h-full border-2 border-[#0A0A0A] text-center transition-all duration-150 rounded-[4px]
        ${isAHold ? 'bg-[#D1D1D1] border-b-2' : 'bg-white border-b-4'}`)}>
            <button
                className="flex flex-col items-center justify-center w-full h-full text-black rounded-[4px] text-center py-[12px] px-[8px] text-[15px] font-medium"
                onMouseDown={() => setIsAHold(true)}
                onMouseUp={() => setIsAHold(false)}
                onMouseLeave={() => setIsAHold(false)}
                onTouchStart={() => setIsAHold(true)}
                onTouchEnd={() => setIsAHold(false)}
        ...
        ${isBHold ? 'bg-[#D1D1D1] border-b-2' : 'bg-white border-b-4'}`)}>
            <button
                ...
                onTouchStart={() => setIsBHold(true)}
                onTouchEnd={() => setIsBHold(false)}
        ...
        ${isCHold ? 'bg-[#D1D1D1] border-b-2' : 'bg-white border-b-4'}`)}>
            <button
                ...
                onTouchStart={() => setIsCHold(true)}
                onTouchEnd={() => setIsCHold(false)}
